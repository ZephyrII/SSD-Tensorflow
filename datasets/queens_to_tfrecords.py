...
from datasets.queen import QUEEN_LABELS
...
def _process_image(directory, name):
    ...
    for obj in root.findall('object'):
        ...
        if label=="queenww":
            label="queen"
        labels.append(int(QUEEN_LABELS[label][0]))
...
def run(dataset_dir, output_dir, name='queen_train', shuffling=False):
    ...
    while i < len(filenames):
        ...
        if i%10>6:
            tf_filename = _get_output_filename(output_dir, name+'_train', fidx)
        else:
            tf_filename = _get_output_filename(output_dir, name+"_test", fidx)
        with tf.python_io.TFRecordWriter(tf_filename) as tfrecord_writer:
            j = 0
            while i < len(filenames) and j < SAMPLES_PER_FILES:
                sys.stdout.write('\r>> Converting image %d/%s' % (i+1, filenames[i]))
                sys.stdout.flush()
    ...
    print('\nFinished converting the queen dataset!')
